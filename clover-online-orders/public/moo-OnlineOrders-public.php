...
class Moo_OnlineOrders_Public {
	...
	public function enqueue_scripts() {
        ...
        // Page URLs
        $cart_page_url    = !empty($MooOptions['cart_page']) ? get_page_link($MooOptions['cart_page']) : '#';
        $checkout_page_url = !empty($MooOptions['checkout_page']) ? get_page_link($MooOptions['checkout_page']) : '#';
        $store_page_url   = !empty($MooOptions['store_page']) ? get_page_link($MooOptions['store_page']) : '#';
    ...
    public function moo_UpdateOrdertype() {
        ...
        if (empty($uuid)) {
            wp_send_json_error(['message' => 'UUID is required'], 400);
            return false;
        }

        $label          = isset($_POST['name']) ? sanitize_text_field($_POST['name']) : '';
        $status         = isset($_POST['enable']) ? filter_var($_POST['enable'], FILTER_VALIDATE_BOOLEAN) : false;
        $taxable        = isset($_POST['taxable']) ? filter_var($_POST['taxable'], FILTER_VALIDATE_BOOLEAN) : false;
        $isDelivery     = isset($_POST['type']) ? filter_var($_POST['type'], FILTER_VALIDATE_BOOLEAN) : false;
        $minAmount      = isset($_POST['minAmount']) ? floatval($_POST['minAmount']) : 0;
        $maxAmount      = isset($_POST['maxAmount']) && $_POST['maxAmount'] !== '' ? floatval($_POST['maxAmount']) : '';
        $customHours    = isset($_POST['availabilityCustomTime']) ? sanitize_text_field($_POST['availabilityCustomTime']) : '';
        $useCoupons     = isset($_POST['useCoupons']) ? filter_var($_POST['useCoupons'], FILTER_VALIDATE_BOOLEAN) : true;
        $allowScOrders  = isset($_POST['allowScOrders']) ? filter_var($_POST['allowScOrders'], FILTER_VALIDATE_BOOLEAN) : true;
        $allowServiceFee = isset($_POST['allowServiceFee']) ? filter_var($_POST['allowServiceFee'], FILTER_VALIDATE_BOOLEAN) : true;
        $customMessage  = isset($_POST['customMessage']) ? wp_kses_post(wp_unslash($_POST['customMessage'])) : '';

        // Validate required fields
        if (empty($label)) {
            wp_send_json_error(['message' => 'Label and Type are required fields'], 400);
            return false;
        }
        // Update the ordertype in the database
        $res = $this->model->updateOrderType(
            $uuid,
            $label,
            $status,
            $taxable,
        ...
        // Update in Clover
        $cloverResponse = $this->api->updateOrderType($uuid, $label, $taxable);
        $result = json_decode($cloverResponse, true);

        // Determine Clover update status
        $updated = !(isset($result['message']) && $result['message'] === 'Not Found');
    ...
    public function moo_ReorderOrderTypes(){
        ...
        $newSortOrder = isset($_POST['newtable']) && is_array($_POST['newtable']) ? array_map('sanitize_text_field', $_POST['newtable']) : null;
        ...
        $res = $this->model->saveNewOrderOfOrderTypes($newSortOrder);
        if ($res){
            $this->api->sendEvent([
                "event"=>'updated-ordertypes'
            ]);
            ...
            wp_send_json_error([
                'status'  => 'error',
                'message' => 'Failed to update order types',
            ], 500);
        }
    }

    /*
     * Function for Importing the Inventory from the plugin settings
    ...
    public function moo_ImportItemsV2() {
        ...
        $savingResult = $this->api->save_items($result);
        $produbtsNb = $this->model->NbProducts();
        $response = array(
            'status'	 => 'Success',
            'received'	 => @count($result),
            'saved'	 => $savingResult['count'],
            'exist'	 => $savingResult['exist'],
    ...
    /**
     * a hook to import the full inventory at once
     * @return void
     */
   ...
   public function moo_SendFeedBack() {
    ...
    public function moo_ChangeModifierName() {
        ...
        $name     = wp_kses_post($_POST['m_name']);
    ...
    public function moo_NewOrderGroupModifier() {
        ...
        $sanitized_array = array_map('sanitize_text_field', $_POST['newtable']);
        $ret = $this->model->saveNewOrderGroupModifier($sanitized_array);
        wp_send_json($ret);
    }
    public function moo_NewOrderModifier(){
        ...
        $sanitized_array = array_map('sanitize_text_field', $_POST['newtable']);
        $ret = $this->model->saveNewOrderModifier($group,$sanitized_array);
        wp_send_json($ret);
    }

    /*
     * Function to manage item's images & description
     * since v1.1.3
     */
    ...
    public function moo_UpdateItems() {
        ...
        if($res){
            ...
            foreach ($res as $item) {
                try {
                    if($this->api->syncCloverItem($item)) {
                        $compteur++;
                    }
                } catch (Exception $e){}
            }
    ...
    public function visibility_category()
    {
        ...
        $id = sanitize_text_field($_POST["id_cat"]);
        $status = rest_sanitize_boolean($_POST["visiblite"]);
    ...
    public function save_image_category(){
        ...
        $uuid = sanitize_text_field($_POST["category_uuid"]);
        $url = sanitize_url($_POST["image"]);
    ...
    public function new_order_categories(){
        ...
        $ret = $this->model->saveNewCategoriesorder($sanitized_array);
        if ($ret){
            $this->api->sendEvent([
    ...
    public function delete_img_category(){
        ...
        $uuid = sanitize_text_field($_POST["uuid"]);
        $ret = $this->model->moo_DeleteImgCategory($uuid);
        if ($ret){
            $this->api->sendEvent([
                "event"=>'updated-category',
                "field"=>'imageUrl',
    ...
    public function moo_reorder_items() {
        ...
        $OrderedItems = isset($_POST['newtable']) && is_array($_POST['newtable'])
            ? array_map('sanitize_text_field', $_POST['newtable'])
            : null;
        $catUuid = isset($_POST['uuid']) ? sanitize_text_field($_POST['uuid']) : null;

        if (empty($OrderedItems) || empty($catUuid)) {
            wp_send_json_error([
                'status'  => 'error',
                'message' => 'Invalid or missing input data',
            ], 400);
        ...
        // Reorder items based on category properties
        $res = (!empty($category->items) && empty($category->items_imported))
            ? $this->model->reOrderItems($OrderedItems)
            : $this->model->reOrderCategoryItems($category->uuid, $OrderedItems);
        if ($res){
            $this->api->sendEvent([
                "event"=>'reorder-items',
                "uuid"=>$catUuid,
    ...
    /*
     * Customer Logins, signup addresses function
     * Used in the Old Checkout Page
     */
